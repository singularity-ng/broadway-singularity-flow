defmodule Broadway.SingularityWorkflowsProducerTest do
  ...
  describe "start_link/1" do
    test "starts GenServer and SingularityWorkflow workflow child", %{workflow_pid: _pid} do
      opts = [
        workflow_name: "test_producer",
        queue_name: "test_queue",
        concurrency: 5,
        batch_size: 10,
        singularity_workflows_config: [timeout_ms: 300_000],
        resource_hints: [gpu: true]
      ]

      assert {:ok, pid} = SingularityWorkflowsProducer.start_link(opts)
      assert is_pid(pid)
    ...
    test "fails with missing required opts" do
      ...
      assert {:error, {:badarg, _}} = SingularityWorkflowsProducer.start_link(opts)
    end
  ...
  describe "handle_demand/3" do
    test "enqueues workflow with demand and returns empty list", %{workflow_pid: workflow_pid} do
      ...
      assert {:noreply, [], ^state} = SingularityWorkflowsProducer.handle_demand(10, 0, state)
    end
    ...
    test "handles high concurrency demand", %{workflow_pid: workflow_pid} do
      ...
      assert {:noreply, [], ^state} = SingularityWorkflowsProducer.handle_demand(100, 0, state)
    end
  end
  ...
  describe "handle_failure/4" do
    test "updates workflow to requeue failed messages", %{workflow_pid: workflow_pid} do
      ...
      assert {:noreply, ^state} = SingularityWorkflowsProducer.handle_failure({:basic, :test_error}, batch, nil, state)
    end
  ...
  describe "handle_info/2" do
    test "pushes yielded messages to Broadway", %{workflow_pid: workflow_pid} do
      ...
      assert {:noreply, ^messages, ^state} = SingularityWorkflowsProducer.handle_info({:workflow_yield, messages}, state)
    end
  ...
  describe "resource hints" do
    test "passes resource hints to workflow state", %{workflow_pid: workflow_pid} do
      ...
      expect(Workflow, :start_link, fn "test_producer", Broadway.SingularityWorkflowsProducer.Workflow, state ->
        assert state.resource_hints == [gpu: true, cpu_cores: 4]
        {:ok, workflow_pid}
      end)

      assert {:ok, _pid} = SingularityWorkflowsProducer.start_link(opts)
    end
  ...
  describe "error scenarios" do
    test "handles workflow start failure", %{workflow_pid: _workflow_pid} do
      ...
      assert {:error, :workflow_failed} = SingularityWorkflowsProducer.start_link(opts)
    end
