defmodule Broadway.QuantumFlowProducerResourceHintTest do
  ...
  test "acquire_hint reserves DB row and caches token" do
    ...
    batches = [
      [
        %Broadway.Message{
          ...
          acknowledger: {Broadway.QuantumFlowProducer.Workflow, 1}
        }
      ]
    ]
    ...
    assert {:next, :wait_acks, new_state} = Broadway.QuantumFlowProducer.Workflow.yield_and_commit(state)
  ...
  test "failed acquire_hint prevents message send and job not yielded" do
    ...
    batches = [
      [
        %Broadway.Message{
          data: {2, %{}},
          metadata: %{resource_key: "gpu"},
          acknowledger: {Broadway.QuantumFlowProducer.Workflow, 2}
        }
      ]
    ]
    ...
    assert {:halt, {:error, {:resource_unavailable, "gpu"}}, ^state} = Broadway.QuantumFlowProducer.Workflow.yield_and_commit(state)
  end
  ...
  test "ack releases hint and marks job completed" do
    ...
    assert {:ok, new_state} = Broadway.QuantumFlowProducer.Workflow.handle_update(:ack, %{id: 3}, state)
