defmodule Broadway.QuantumFlowProducerIntegrationTest do
  ...
  describe "end-to-end demand → yield → ack" do
    test "fetches from mock queue, yields messages, and acks update status", %{workflow_pid: workflow_pid} do
      ...
      expect(Workflow, :enqueue, fn ^workflow_pid, :fetch, %{demand: 1, batch_size: 1} ->
        ...
        messages = [
          %Message{
            data: {1, :job1},
            metadata: %{},
            acknowledger: {QuantumFlowProducer.Workflow, 1}
          }
        ]
      ...
      # Simulate ack
      ack_msg = %Message{data: {1, :job1}, acknowledger: {QuantumFlowProducer.Workflow, 1}}
      QuantumFlowProducer.Workflow.handle_update(:ack, %{id: 1}, %{})
  ...
  describe "atomic yield_and_commit" do
        ...
        try do
          ...
          messages = [
            %Message{
              data: {42, :job42},
              metadata: %{},
              acknowledger: {QuantumFlowProducer.Workflow, 42}
            }
          ]
          send(producer_pid, {:workflow_yield, messages})
        rescue
    ...
    test "yield_and_commit succeeds: messages sent and jobs marked in_progress", %{workflow_pid: workflow_pid} do
          ...
          {1, _} ->
            messages = [
              %Message{
                data: {7, :job7},
                metadata: %{},
                acknowledger: {QuantumFlowProducer.Workflow, 7}
              }
            ]
            send(producer_pid, {:workflow_yield, messages})
          other ->
      ...
      expect(Broadway, :push_messages, 1, fn Broadway.QuantumFlowProducer, messages ->
  ...
  describe "stress tests" do
    ...
    describe "dynamic batching" do
      test "adjusts batch_size downward under high queue depth", %{workflow_pid: workflow_pid} do
        ...
        expect(Broadway, :push_messages, expected_batches, fn Broadway.QuantumFlowProducer, messages ->
  ...
  describe "error recovery tests" do
    ...
    test "recovers from partial batch failure", %{workflow_pid: workflow_pid} do
      ...
      QuantumFlowProducer.handle_failure({:basic, :processing_error}, failed_batch, nil, %{workflow_pid: workflow_pid})

      # Verify other jobs can still be acked
      QuantumFlowProducer.Workflow.handle_update(:ack, %{id: 1}, %{})
      QuantumFlowProducer.Workflow.handle_update(:ack, %{id: 3}, %{})
